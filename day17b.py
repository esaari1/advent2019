...
def handleInput(val):
	global command, prompt, inputIdx, running
	...
	if prompt == 'Main:':
		command = main
	elif prompt == 'Function A:':
		command = a
	elif prompt == 'Function B:':
		command = b
	elif prompt == 'Function C:':
		command = c
	elif prompt == 'Continuous video feed?':
		command = feed
		running = True

	if inputIdx < len(command):
		input[val] = ord(command[inputIdx])
		print command[inputIdx]
		inputIdx += 1
	else:
		input[val] = 10
		inputIdx = 0
