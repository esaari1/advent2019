...
plot1.sort(key = lambda x: mdist(x))
plot2.sort(key = lambda x: mdist(x))

for p in plot1:
	mp = mdist(p)
	if mp > 0:
		result = binarySearch(plot2, 0, len(plot2)-1, mp)
		if result != -1:
			while mp == mdist(plot2[result]):
				if p == plot2[result]:
					print p, plot2[result]
					print "ANS: ", mdist(p)
					exit(0)
				result += 1
