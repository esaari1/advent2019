...
def plot(line1):
	...
	for part in line1.split(","):
		...
		if dir == "R":
			for i in range(step):
				pos1.append([start[0] + i + 1, start[1]])
			...
			for i in range(step):
				pos1.append([start[0], start[1] + i + 1])
...
sort1.sort(key = lambda x: mdist(x))
sort2.sort(key = lambda x: mdist(x))
...
for p in sort1:
	mp = mdist(p)
	if mp > 0:
		result = binarySearch(sort2, 0, len(sort2)-1, mp)
		if result != -1:
			while result < len(sort2) and mp == mdist(sort2[result]):
				if p == sort2[result]:
					s = stepCount(p, plot1) + stepCount(p, plot2)
					if minSteps is None or minSteps > s:
						minSteps = s
					break
